<!DOCTYPE html>
<html>
<head>
	<title>Formulario</title>
	<meta name="viewport" content="width=device-width, initial-scale=1.0, shrink-to-fit=no" />
    <link href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/5.13.0/css/all.min.css" rel="stylesheet">
    <link rel="stylesheet" type="text/css" href="/assets-web/css/bootstrap.min.css">
    <link rel="stylesheet" type="text/css" href="/assets-web/css/estilo.css">
    <link rel="stylesheet" type="text/css" href="/assets-web/css/magic-input.min.css">
    <link rel="stylesheet" type="text/css" href="/assets-web/css/animations.css">
</head>
<body style="background-color: #FBFBFB">
	<section class="position-relative">
		<div class="div_blanco_cabezera mt-4"></div>
		<img src="/assets-web/img/Grupo721.png" class="img_logo_cabezera">
	</section>
	<section class="secc_bajar_primero">
		<div class="container">
			<h6 class="subtitulo_heavy_33 text-center mb-5">More about me</h6>
			<div class="mb-5 w-100 px-3 px-sm-5 div_blanco_texte pt-3 pb-5 position-relative">
				<p class="p_textarea my-0">About me</p>
				<textarea class="textarea_form" placeholder="Tell the girls what’s so great about you, make ‘em like you."></textarea>
				<p class="cantidad_caracteres my-0">81/1000</p>
			</div>
			<h6 class="subtitulo_heavy_33 text-center mb-5">Appearance</h6>
			<div class="row mx-0 mb-5 justify-content-start">
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Gender</p>
					<select class="select_form_1">
						<option>USA</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Body Type</p>
					<select class="select_form_1">
						<option>New York</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Height</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Ethnicity</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Hair Color</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Eye Color</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Drinking</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Smoking</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
			</div>
			<h6 class="subtitulo_heavy_33 text-center mb-5">Personality</h6>
			<div class="row mx-0 mb-5 justify-content-start">
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Interested In</p>
					<select class="select_form_1">
						<option>USA</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Orientation</p>
					<select class="select_form_1">
						<option>New York</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Relationship</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Sexually</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Education</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Occupation</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Religion</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Children</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
			</div>
			<h6 class="subtitulo_heavy_33 text-center mb-5">Languages</h6>
			<div class="row mx-0 mb-5 justify-content-start">
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Language 1</p>
					<select class="select_form_1">
						<option>USA</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Language 2</p>
					<select class="select_form_1">
						<option>New York</option>
					</select>
				</div>
				<div class="col-md-6 col-xl-4 d-flex flex-wrap justify-content-center align-items-center mb-4">
					<p class="my-0 p_into w_p_fomr2">Language 3</p>
					<select class="select_form_1">
						<option>Queens</option>
					</select>
				</div>
			</div>
			<div class="row mx-0 mt-5 justify-content-center">
				<div class="col-sm-6 col-md-5 col-lg-4 text-center mt-5">
					<a href="" class="btn btn_next_form">Next</a>
				</div>
				<div class="col-12 text-center mt-5">
					<a href="" class="link_step"><u>Skip step ></u></a>
				</div>
			</div>
		</div>
	</section>
	<footer class="footer_form mt-5 py-3">
		<div class="container px-sm-0">
			<div class="d-flex justify-content-center justify-content-lg-between flex-wrap align-items-center">
				<div class="d-flex align-items-end flex-wrap justify-content-center justify-content-lg-start div_logo_pro">
					<a href="">
						<div class="div_logo">
							<img src="/assets-web/img/Logoh60.png">
						</div>
					</a>
					<p class="my-0 p_2020_footer ml-0 mt-3 mt-lg-0">© 2020. Seekingterms.</p>
				</div>
				<ul class="d-flex flex-wrap justify-content-center mb-0 pl-0 mt-4 mt-lg-0" style="list-style-type: none;">
					<li class="mr-3 mr-sm-3 mr-md-4"><a href="" class="link_footerfomr1">HOW IT WORKS</a></li>
					<li class="mr-3 mr-sm-3 mr-md-4"><a href="" class="link_footerfomr1">FAQS</a></li>
					<li class="mr-3 mr-sm-3 mr-md-4"><a href="" class="link_footerfomr1">BROWSE</a></li>
					<li class="mr-3 mr-sm-3 mr-md-4"><a href="" class="link_footerfomr1">BLOG</a></li>
					<li class="mr-3 mr-sm-3 mr-md-4"><a href="" class="link_footerfomr1">AFFILIATE</a></li>
					<li class="mr-3 mr-sm-3 mr-md-4"><a href="" class="link_footerfomr1">TERMS</a></li>
					<li class=""><a href="" class="link_footerfomr1">PRIVACY</a></li>
				</ul>
			</div>
		</div>
	</footer>
<script src="/assets-web/js/jquery.min.js"></script>
<script src="/assets-web/js/bootstrap.min.js"></script>
<script src="/assets-web/js/css3-animate-it.js"></script>
<script src="/assets-web/js/canvas.js"></script>
</body>
</html>